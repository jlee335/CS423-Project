...
n_steps = 2 if smoke_test else 30000
...
prior_init = prior  #torch.add(prior,prior_nil)
prior_init_guide = prior#torch.tensor(np.zeros((prior.shape[0],prior.shape[1])))
...
def model(data):
    ...
    ##############################################################
    #alpha = alpha_prior#pyro.sample('alpha',    dist.Normal(alpha_prior,1))
    #beta  = beta_prior#pyro.sample('beta',     dist.Normal(beta_prior,1))
    #gamma = gamma_prior#pyro.sample('gamma',    dist.Normal(gamma_prior,1))
    

    #x_t = t_prior[0]#pyro.sample('x_t', dist.Normal(t_prior[0],0.5))
    #y_t = t_prior[1]#pyro.sample('y_t', dist.Normal(t_prior[1],0.5))
    #z_t = t_prior[2]#pyro.sample('z_t', dist.Normal(t_prior[2],0.5))
...
def guide(data):
    ...
    #alpha_q =   alpha_prior      #pyro.param("alpha_q",torch.tensor(alpha_prior + 0.1))
    #beta_q =    beta_prior    #pyro.param("beta_q",torch.tensor(beta_prior+ 0.1)   )
    #gamma_q =   gamma_prior    #pyro.param("gamma_q",torch.tensor(gamma_prior+ 0.1) )
    #x_t_q =     t_prior[0]    #pyro.param("x_t_q",torch.tensor(guide_x)                  )
    #y_t_q =     t_prior[1]    #pyro.param("y_t_q",torch.tensor(guide_y)                  )
    #z_t_q =     t_prior[2]    #pyro.param("z_t_q",torch.tensor(guide_z)                 )

    #alpha = alpha_prior           #pyro.sample('alpha', dist.Normal(alpha_q,0.1))
    #beta = beta_prior             #pyro.sample('beta', dist.Normal(beta_q,0.1))
    #gamma =   gamma_prior         #pyro.sample('gamma', dist.Normal(gamma_q,0.1))
    #x_t =     t_prior[0]          #pyro.sample('x_t', dist.Normal(x_t_q,0.5))
    #y_t =     t_prior[1]          #pyro.sample('y_t', dist.Normal(y_t_q,0.5))
    #z_t =     t_prior[2]          #pyro.sample('z_t', dist.Normal(z_t_q,0.5))
    #Construct P using info sampled above
    #R = torch.from_numpy(eulerAnglesToRotationMatrix(np.array([alpha,beta,gamma]))).float() 
