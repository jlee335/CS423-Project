...
alpha = 0.0 #alpha_prior
beta =  0.0 #beta_prior
gamma = 0.0 #gamma_prior
...
prior_init_guide = torch.tensor(np.ones((prior.shape[0],prior.shape[1])))
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        (alpha,beta,gamma,x_t,y_t,z_t,X) = x
...
class VAE(nn.Module):
    ...
    def model(self, x):
        ...
        # Construct result matrix using these parameters.
        #R = torch.from_numpy(eulerAnglesToRotationMatrix(np.array([alpha,beta,gamma]))).float() 
        #t = torch.tensor([[x_t],[y_t],[z_t]]).float()                     #3x1
        #extr_R = torch.cat((R,t),1) 
        #P_R = torch.mm(torch.from_numpy(K).float(),extr_R)
        #Px =  torch.from_numpy(np.concatenate((P_L,P_R),axis = 0)).float()     
        
        #alpha beta gamma x_t y_t z_t X 들을 NN 으로 보내버리자

        #res = torch.mm(Px,X)
        res = self.decoder.forward((alpha,beta,gamma,x_t,y_t,z_t,X))
